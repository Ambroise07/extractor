...
class InputesUI(tkinter.Frame):
    ...
    def __init__(
        self,
        frame: tkinter.Frame | tkinter.Tk,
        src_ext: list[str],
        dest_ext: list[str],
        **args,
    ):
        ...
        # source inpute
        self.src_input = tkinter.Entry(
            master=src_master, textvariable=self.src, width=30
        )
        self.src_input.pack(side="left")

        # source button
        src_button = tkinter.Button(
            master=src_master, text="choisir", command=self.choose_src
        )
        ...
        # destination inpute
        self.dest_input = tkinter.Entry(
            master=dest_master, textvariable=self.dest, width=30
        )
        self.dest_input.pack(side="left")

        # destination button
        dest_button = tkinter.Button(
            master=dest_master, text="choisir", command=self.choose_dest
        )
    ...
    def choose_src(self):
        ...
        file_path = filedialog.askopenfilename(
            title="selectionner le fichier source", filetypes=type
        )

        # empty file_path : user clicked cancel bouton
        if not file_path:
          return None
          
        # check extension here
        ext = [True for ext in self.src_ext if self.check_extension(file_path, ext)]
    ...
    def choose_dest(self):
        ...
        file_path = filedialog.askopenfilename(
            title="selectionner le fichier de destination", filetypes=type
        )
        ...
        # check extension here
        ext = [True for ext in self.dest_ext if self.check_extension(file_path, ext)]
...
class ProgressUI:
    ...
    def __init__(self, root: tkinter.Tk | tkinter.Frame):
        ...
        if not (
            hasattr(root, "perform_task") and isinstance(root.perform_task, Callable)
        ):
        ...
        main_master.pack(
            side="top", expand=True, fill="both", pady=("1c", "1c"), padx=("1c", "1c")
        )
        ...
        progressbar = ttk.Progressbar(
            master=progressbar_master, mode="determinate", maximum=root.max_val
        )
        ...
        # open button
        self.cancel_or_open_button = tkinter.Button(
            master=buttons_master, text="annuler", command=self.callback
        )
    ...
    def execute_task(self):
        ...
        self.progressbar["value"]: int = (
            task_performed if task_performed is not None else 0
        )
...
class TextExtractor(tkinter.Frame):
    ...
    def __init__(self, root: tkinter.Tk, **args):
        ...
        self.submit_button = tkinter.Button(
            master=submit_master, text="valider", command=self.show_progressui
        )
    ...
    def show_progressui(self, **args):
        ...
        match self.src_file.endswith(".pdf"):
            case True:
                extractor = logic.PDFExtractor(
                    src=self.src_file, dest=self.temp, callback=self.perform_task
                )

                steps = extractor.async_extract_text()

            case False:
                extractor = logic.DOCXExtractor(
                    src=self.src_file, dest=self.temp_file, callback=self.perform_task
                )
    ...
    def cancel_task(self, *args):
        ...
        can_cancel = askokcancel(
            title="confirmation d'annulation",
            message="Voulez-vous annuler l'extraction en cours ?",
        )
...
class TextExtractorApp(tkinter.Tk):

    """
    For a demo use this classe.
    to have the extractor plugin into 
    your app, please use an instance of the next class.
    ...
    def create_text_widget(self):
        ...
        if self.text_content:
            ...
            # close this window
            close_button = tkinter.Button(master=self, text="fermer", 
                                    command=self.destroy)
            close_button.pack(side="bottom", padx=("2c", 0))



class TextExtractorInterface(tkinter.Frame):
    """
    ...
    """
    def __init__(self, root:tkinter.Tk | tkinter.Frame, *args) -> None:
        super().__init__(root)
    ...
    def create_text_widget(self):
        ...
        # close this window
        close_button_master = tkinter.Frame(self)
        close_button_master.pack(
                       side="bottom", 
                       expand=True, 
                       fill="x", 
                       padx="2c", 
